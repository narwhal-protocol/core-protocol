...
contract PriceOracleProxy is PriceOracle {
    ...
    /// @notice Address of the nBnb contract, which has a constant price
    address public nBnbAddress;
    ...
    function getUnderlyingPrice(NToken nToken) public view returns (uint) {
        ...
        if (nTokenAddress == nBnbAddress) {
            // bnb always worth 1
            return 1e18;
        }

        if (nTokenAddress == vUsdcAddress || nTokenAddress == vUsdtAddress) {
            return v1PriceOracle.assetPrices(usdcOracleKey);
        }

        if (nTokenAddress == vDaiAddress) {
            return v1PriceOracle.assetPrices(daiOracleKey);
        }

        if (nTokenAddress == vSaiAddress) {
        ...
        address underlying = NBep20(nTokenAddress).underlying();
