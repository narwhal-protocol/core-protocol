...
import "./NWLVaultProxy.sol";
import "./NWLVaultStorage.sol";
import "./NWLVaultErrorReporter.sol";
...
contract NWLVault is NWLVaultStorage, ECDSA {
    ...
    event StoreUpdated(address oldNwl, address oldStore, address newNwl, address newStore);
    ...
    function add(
        ...
        onlyAdmin
    {
        require(address(nwlStore) != address(0), "Store contract addres is empty");
        ...
        INWLStore(nwlStore).setRewardToken(_rewardToken, true);
    ...
    /**
     * @notice Deposit NWLVault for NWL allocation
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            ...
            INWLStore(nwlStore).safeRewardTransfer(_rewardToken, msg.sender, pending);
        }
        ...
        if (address(pool.token) == address(nwlAddress)) {
    ...
    /**
     * @notice Execute withdrawal to NWLVault for NWL allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
     */
    ...
    /**
     * @notice Request withdrawal to NWLVault for NWL allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
        ...
        nonReentrant
    {
        ...
        // Update Delegate Amount
        if (_rewardToken == address(nwlAddress)) {
    ...
    // View function to see pending NWLs on frontend.
    ...
    // Update reward nairables for all pools. Be careful of gas spending!
    ...
    /**
     * @notice Get the NWL stake balance of an account (excluding the pending withdrawals)
     * @param account The address of the account to check
     * @return The balance that user staked
     */
    function getStakeAmount(address account) internal view returns (uint96) {
        require(nwlAddress != address(0), "NWLVault::getStakeAmount: nwl address is not set");

        PoolInfo[] storage poolInfo = poolInfos[nwlAddress];

        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            if (address(poolInfo[pid].token) == address(nwlAddress)) {
                UserInfo storage user = userInfos[nwlAddress][pid][account];
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s)
        external
    {
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("NWLVault")), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(DELEGATION_TYPEHASH, delegatee, nonce, expiry));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ECDSA.recover(digest, v, r, s);
        require(nonce == nonces[signatory]++, "NWLVault::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "NWLVault::delegateBySig: signature expired");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "NWLVault::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "NWLVault::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "NWLVault::_writeCheckpoint: block number exceeds 32 bits");
    ...
    /**
     * @notice Determine the nwl stake balance for an account
     * @param account The address of the account to check
     * @param blockNumber The block number to get the vote balance at
     * @return The balance that user staked
     */
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint96) {
        require(blockNumber < block.number, "NWLVault::getPriorVotes: not yet determined");
    ...
    function _become(NWLVaultProxy nwlVaultProxy) external {
        require(msg.sender == nwlVaultProxy.admin(), "only proxy admin can change brains");
        require(nwlVaultProxy._acceptImplementation() == 0, "change not authorized");
    }

    function setNwlStore(address _nwl, address _nwlStore) external onlyAdmin {
        address oldNwlContract = nwlAddress;
        address oldStore = nwlStore;
        nwlAddress = _nwl;
        nwlStore = _nwlStore;

        _notEntered = true;

        emit StoreUpdated(oldNwlContract, oldStore, _nwl, _nwlStore);
    }
}
