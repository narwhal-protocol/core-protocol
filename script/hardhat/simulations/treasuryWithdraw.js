...
async function treasuryWithdraw() {
  ...
  await treasuryContract.methods.withdrawTreasuryBEP20(nwl, nwlBalance, guy).send({
    from: timelock,
  });
  ...
  const guyNwlBalance = await nwlToken.methods.balanceOf(guy).call();
  ...
  expect(guyNwlBalance).to.equal(nwlBalance.toString(10));
