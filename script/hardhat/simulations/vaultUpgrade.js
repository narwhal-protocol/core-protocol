...
async function upgradeVault() {
  ...
  console.log("Deploying NWLVault...");
  ...
  const nwlAddress = await vault.methods.nwlAddress().call();
  console.log("XVS address:", nwlAddress);
  ...
  async function testWithdrawals() {
    ...
    const tx3 = await vault.methods.requestWithdrawal(nwlAddress, 0, "2100000000000000000").send({ from: team });
    const newDeployerVotes = await vault.methods.getCurrentVotes(deployer).call();
    console.log("Deployer votes after withdrawal:", newDeployerVotes);
    expect(await vault.methods.getUserInfo(nwlAddress, 0, team).call()).to.deep.include({
