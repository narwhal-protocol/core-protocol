...
import { processNTokenDelegateEvent, vTokenDelegateCommands } from "./Event/NTokenDelegateEvent";
import { processNTokenEvent, vTokenCommands } from "./Event/NTokenEvent";
import { processXVSEvent, nwlCommands } from "./Event/XVSEvent";
import { processXVSVaultEvent, nwlVaultCommands } from "./Event/XVSVaultEvent";
import { processXVSVaultImplEvent, nwlVaultImplCommands } from "./Event/XVSVaultImplEvent";
import { processXVSVaultProxyEvent, nwlVaultProxyCommands } from "./Event/XVSVaultProxyEvent";
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Trx ...trxEvent" - Handles event to set details of next transaction
        * E.g. "Trx Value 1.0e18 (NToken vBnb Mint 1.0e18)"
    `,
    "Trx",
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Expect ...expectation" - Adds an expectation to hold after the next transaction
        * E.g. "Expect Changes (NToken vZRX TotalSupply) +10.0e18"
    `,
    "Expect",
    [new Arg("event", getEventV, { variadic: true })],
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "PriceOracleProxy Deploy (Unitroller Address) (PriceOracle Address) (NToken vBNB Address)"
    `,
    "PriceOracleProxy",
    [new Arg("event", getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "Maximillion Deploy (NToken vBNB Address)"
    `,
    "Maximillion",
    [new Arg("event", getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Liquidator ...event" - Narwhal v3 liquidation interface
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "XVS ...event" - Runs given nwl event
      * E.g. "XVS Deploy"
    `,
    "XVS",
    [new Arg("event", getEventV, { variadic: true })],
  ...
  new Command<{ event: EventV }>(
    ...
    { subExpressions: nwlVaultCommands() },
  ),
  ...
  new Command<{ event: EventV }>(
    ...
    { subExpressions: nwlVaultImplCommands() },
  ),
  ...
  new Command<{ event: EventV }>(
    ...
    { subExpressions: nwlVaultProxyCommands() },
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "VAI ...event" - Runs given nwl event
      * E.g. "VAI Deploy"
    `,
    "VAI",
    [new Arg("event", getEventV, { variadic: true })],
  ...
  buildContractEvent("NarwhalLens", false),
