...
import { getNTokenDelegateValue, vTokenDelegateFetchers } from "./Value/NTokenDelegateValue";
import { getNTokenValue, vTokenFetchers } from "./Value/NTokenValue";
import { getXVSValue, nwlFetchers } from "./Value/XVSValue";
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case "marketStruct": {
          ...
          const isNarwhalStr = await world.web3.eth.getStorageAt(addr.val, isCompKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isNarwhalStr, 1)),
          ]);
        }
        default:
          return new NothingV();
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (NToken vZRX TotalSupply) (Exactly 55)"
        * E.g. "Equal (NToken vZRX Comptroller) (Comptroller Address)"
    `,
    "Equal",
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "NToken ...vTokenArgs" - Returns vToken value
    `,
    "NToken",
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "NTokenDelegate ...vTokenDelegateArgs" - Returns vToken delegate value
    `,
    "NTokenDelegate",
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "XVS ...narwhalArgs" - Returns XVS value
    `,
    "XVS",
    [new Arg("res", getXVSValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: nwlFetchers() },
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### SXP

      * "SXP ...narwhalArgs" - Returns SXP value
    `,
    "SXP",
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "VAI ...narwhalArgs" - Returns VAI value
    `,
    "VAI",
...
const contractFetchers = [
  ...
  { contract: "NarwhalLens", implicit: false },
