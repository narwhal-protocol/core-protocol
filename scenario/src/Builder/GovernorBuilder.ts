...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<{ name: StringV; timelock: AddressV; nwl: AddressV; guardian: AddressV }, GovernorData>(
      `
      #### GovernorAlpha

      * "Governor Deploy Alpha name:<String> timelock:<Address> nwl:<Address> guardian:<Address>" - Deploys Narwhal Governor Alpha
    ...
    `,
      ...
      async (world, { name, timelock, nwl, guardian }) => {
        return {
          invokation: await GovernorAlphaContract.deploy<Governor>(world, from, [timelock.val, nwl.val, guardian.val]),
          name: name.val,
    ...
    new Fetcher<{ name: StringV; timelock: AddressV; nwl: AddressV; guardian: AddressV }, GovernorData>(
      ...
      * "Governor Deploy AlphaHarness name:<String> timelock:<Address> nwl:<Address> guardian:<Address>" - Deploys Narwhal Governor Alpha with a mocked voting period
