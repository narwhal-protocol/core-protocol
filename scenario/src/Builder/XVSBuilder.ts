...
export async function buildXVS(
  ...
  from: string,
  params: Event,
): Promise<{ world: World; nwl: XVS; tokenData: TokenData }> {
  const fetchers = [
  ...
  const nwl = invokation.value!;
  tokenData.address = nwl._address;

  world = await storeAndSaveContract(world, nwl, "XVS", invokation, [
