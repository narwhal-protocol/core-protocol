...
async function genXVSVault(world: World, from: string, params: Event): Promise<World> {
  ...
  world = addAction(
    world,
    `Deployed XVS Vault implementation (${nwlVaultImpl.name}) to address ${nwlVaultImpl._address}`,
...
export function nwlVaultImplCommands() {
  return [
...
export async function processXVSVaultImplEvent(world: World, event: Event, from: string | null): Promise<World> {
  return await processCommandEvent<any>("XVSVaultImpl", nwlVaultImplCommands(), world, event, from);
}
