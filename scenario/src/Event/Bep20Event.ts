...
async function approve(world: World, from: string, bep20: Bep20, address: string, amount: NumberV): Promise<World> {
  const invokation = await invoke(world, bep20.methods.approve(address, amount.encode()), from, NTokenErrorReporter);
...
async function faucet(world: World, from: string, bep20: Bep20, address: string, amount: NumberV): Promise<World> {
  const invokation = await invoke(world, bep20.methods.allocateTo(address, amount.encode()), from, NTokenErrorReporter);
...
async function transfer(world: World, from: string, bep20: Bep20, address: string, amount: NumberV): Promise<World> {
  const invokation = await invoke(world, bep20.methods.transfer(address, amount.encode()), from, NTokenErrorReporter);
...
async function setFail(world: World, from: string, bep20: Bep20, fail: boolean): Promise<World> {
  const invokation = await invoke(world, bep20.methods.setFail(fail), from, NTokenErrorReporter);
