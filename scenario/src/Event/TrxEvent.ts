...
import { getEventV, getNumberV } from "../CoreValue";
import { encodedNumber } from "../Encoding";
import { Event } from "../Event";
import { EventV, NumberV } from "../Value";
    ...
    new Command<{ amount: NumberV; event: EventV }>(
      `
        ...
        * "Value <Amount> <Event>" - Runs event with a set amount for any transactions
          * E.g. "Value 1.0e18 (NToken vBnb Mint 1.0e18)"
      `,
      "Value",
      [new Arg("amount", getNumberV), new Arg("event", getEventV)],
      async (world, from, { amount, event }) =>
        processCoreEvent(await setTrxValue(world, amount.encode()), event.val, from),
    ),
    new Command<{ gasPrice: NumberV; event: EventV }>(
      `
        ...
        * "GasPrice <Amount> <Event>" - Runs event with a given gas price
          * E.g. "GasPrice 0 (NToken vBnb Mint 1.0e18)"
      `,
      "GasPrice",
      [new Arg("gasPrice", getNumberV), new Arg("event", getEventV)],
      async (world, from, { gasPrice, event }) =>
        processCoreEvent(await setTrxGasPrice(world, gasPrice.encode()), event.val, from),
    ),
  ];
}
