...
export function printHelp(printer: Printer, event: Event, expressions: Expression<any>[], path: string[] = []) {
  if (event.length === 0) {
    ...
    if (path.length === 0) {
      banner = `
## Narwhal Command Runner

The Narwhal Command Runner makes it easy to interact with Narwhal. You can input simple commands
...
sub-commands run \`Help NToken\` or \`Help NToken Mint\`.
`.trim();
    } else {
