...
export function getNTokenAddress(world: World, vTokenArg: string): string {
  return getContractDataString(world, [["vTokens", vTokenArg, "address"]]);
}

export function getNTokenDelegateAddress(world: World, vTokenDelegateArg: string): string {
  return getContractDataString(world, [["NTokenDelegate", vTokenDelegateArg, "address"]]);
}
...
export async function getXVSVaultImpl(world: World, nwlVaultImplArg: Event): Promise<XVSVaultImpl> {
  return getWorldContract(world, [["XVSVault", mustString(nwlVaultImplArg), "address"]]);
}
...
export async function getXVS(world: World, _narwhalArg: Event): Promise<XVS> {
  return getWorldContract(world, [["XVS", "address"]]);
}

export async function getXVSData(world: World, narwhalArg: string): Promise<[XVS, string, Map<string, string>]> {
  const contract = await getXVS(world, <Event>(<any>narwhalArg));
  const data = getContractData(world, [["XVS", narwhalArg]]);

  return [contract, narwhalArg, <Map<string, string>>(<any>data)];
}

export async function getSXP(world: World, _narwhalArg: Event): Promise<SXP> {
  return getWorldContract(world, [["SXP", "address"]]);
}

export async function getSXPData(world: World, narwhalArg: string): Promise<[SXP, string, Map<string, string>]> {
  const contract = await getSXP(world, <Event>(<any>narwhalArg));
  const data = getContractData(world, [["SXP", narwhalArg]]);
...
export async function getVAIData(world: World, narwhalArg: string): Promise<[VAI, string, Map<string, string>]> {
  const contract = await getVAI(world, <Event>(<any>narwhalArg));
  const data = getContractData(world, [["VAI", narwhalArg]]);
...
export async function getNTokenData(world: World, vTokenArg: string): Promise<[NToken, string, Map<string, string>]> {
  const contract = getWorldContract<NToken>(world, [["vTokens", vTokenArg, "address"]]);
  const data = getContractData(world, [["NTokens", vTokenArg]]);
...
export async function getNTokenDelegateData(
  ...
  const contract = getWorldContract<VBep20Delegate>(world, [["NTokenDelegate", vTokenDelegateArg, "address"]]);
  const data = getContractData(world, [["NTokenDelegate", vTokenDelegateArg]]);
