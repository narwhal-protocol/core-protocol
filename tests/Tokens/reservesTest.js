const { bnbUnsigned, bnbMantissa, both } = require("../Utils/BSC");

const { fastForward, makeNToken, setMarketSupplyCap } = require("../Utils/Narwhal");

const factor = bnbMantissa(0.02);
...
describe("NToken", function () {
  ...
  describe("_setReserveFactorFresh", () => {
    ...
    it("rejects change by non-admin", async () => {
      expect(await send(vToken, "harnessSetReserveFactorFresh", [factor], { from: accounts[0] })).toHaveTokenFailure(
        "UNAUTHORIZED",
    ...
    it("rejects change if market not fresh", async () => {
      ...
      expect(await send(vToken, "harnessSetReserveFactorFresh", [factor])).toHaveTokenFailure(
        "MARKET_NOT_FRESH",
    ...
    it("rejects newReserveFactor that descales to 1", async () => {
      expect(await send(vToken, "harnessSetReserveFactorFresh", [bnbMantissa(1.01)])).toHaveTokenFailure(
        "BAD_INPUT",
  ...
  describe("_reduceReservesFresh", () => {
    ...
    beforeEach(async () => {
      ...
      expect(await send(vToken.underlying, "harnessSetBalance", [vToken._address, cash])).toSucceed();
    });

    it("fails if called by non-admin", async () => {
      expect(await send(vToken, "harnessReduceReservesFresh", [reduction], { from: accounts[0] })).toHaveTokenFailure(
        "UNAUTHORIZED",
    ...
    it("fails if market not fresh", async () => {
      expect(await send(vToken, "harnessFastForward", [5])).toSucceed();
      expect(await send(vToken, "harnessReduceReservesFresh", [reduction])).toHaveTokenFailure(
        "MARKET_NOT_FRESH",
    ...
    it("fails if amount exceeds reserves", async () => {
      expect(await send(vToken, "harnessReduceReservesFresh", [reserves.add(1)])).toHaveTokenFailure(
        "BAD_INPUT",
    ...
    it("fails if amount exceeds available cash", async () => {
      ...
      expect(await send(vToken, "harnessReduceReservesFresh", [reserves])).toHaveTokenFailure(
