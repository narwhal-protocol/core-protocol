...
const { makeNToken, setBorrowRate, pretendBorrow, setMarketSupplyCap } = require("../Utils/Narwhal");
...
describe("NToken", function () {
  ...
  describe("constructor", () => {
    ...
    it("fails when 0 initial exchange rate", async () => {
      await expect(makeNToken({ exchangeRate: 0 })).rejects.toRevert(
    ...
    it("succeeds when setting admin to contructor argument", async () => {
      const vToken = await makeNToken({ admin: admin });
  ...
  describe("name, symbol, decimals", () => {
    ...
    beforeEach(async () => {
      vToken = await makeNToken({ name: "NToken Foo", symbol: "cFOO", decimals: 10 });
      await setMarketSupplyCap(vToken.comptroller, [vToken._address], [100000000000]);
    });

    it("should return correct name", async () => {
      expect(await call(vToken, "name")).toEqual("NToken Foo");
    });
  ...
  describe("balanceOfUnderlying", () => {
    it("has an underlying balance", async () => {
      const vToken = await makeNToken({ supportMarket: true, exchangeRate: 2 });
      await setMarketSupplyCap(vToken.comptroller, [vToken._address], [100000000000]);
  ...
  describe("borrowBalanceStored", () => {
    ...
    beforeEach(async () => {
      borrower = accounts[0];
      vToken = await makeNToken({ comptrollerOpts: { kind: "bool" } });
    });
  ...
  describe("exchangeRateStored", () => {
    ...
    beforeEach(async () => {
      vToken = await makeNToken({ exchangeRate });
      await setMarketSupplyCap(vToken.comptroller, [vToken._address], [100000000000]);
    });
