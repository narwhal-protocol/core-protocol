...
describe("NToken", function () {
  ...
  beforeEach(async () => {
    ...
    payer = minter;
    vToken = await makeNToken({ comptrollerOpts: { kind: "bool" }, exchangeRate });
  });
  ...
  describe("mintFresh", () => {
    ...
    it("transfers the underlying cash from payer, tokens to receiver, and emits MintBehalf, Transfer events", async () => {
      ...
      expect(payerAfterBalances).toEqual(
        await adjustBalances(payerBeforeBalances, [
      ...
      expect(receiverAfterBalances).toEqual(
        await adjustBalances(receiverBeforeBalances, [
