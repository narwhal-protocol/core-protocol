...
describe("admin / _setPendingAdmin / _acceptAdmin", () => {
  ...
  describe("_setPendingAdmin()", () => {
    it("should only be callable by admin", async () => {
      expect(await send(vToken, "_setPendingAdmin", [accounts[0]], { from: accounts[0] })).toHaveTokenFailure(
        "UNAUTHORIZED",
  ...
  describe("_acceptAdmin()", () => {
    it("should fail when pending admin is zero", async () => {
      expect(await send(vToken, "_acceptAdmin")).toHaveTokenFailure("UNAUTHORIZED", "ACCEPT_ADMIN_PENDING_ADMIN_CHECK");
