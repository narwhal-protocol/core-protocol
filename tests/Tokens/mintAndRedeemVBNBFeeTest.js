...
const { makeNToken, fastForward, setBalance, setBNBBalance, getBalances, adjustBalances } = require("../Utils/Narwhal");
...
async function redeemNTokens(vToken, redeemer, redeemTokens) {
...
describe("VBNB", () => {
  ...
  beforeEach(async () => {
    ...
    vToken = await makeNToken({ kind: "vbnb", comptrollerOpts: { kind: "boolFee" } });
      ...
      it("returns success from redeemFresh and redeems the correct amount", async () => {
        ...
        expect(afterBalances).toEqual(
          await adjustBalances(beforeBalances, [
            ...
            [vToken, redeemer, "bnb", redeemedAmount.sub(await bnbGasCost(receipt))],
