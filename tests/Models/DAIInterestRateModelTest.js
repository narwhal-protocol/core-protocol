...
const { getBorrowRate, getSupplyRate } = require("../Utils/Narwhal");
...
function baseRoofRateFn(dsr, duty, mkrBase, jump, kink, cash, borrows, reserves) {
  ...
  let baseRatePerBlock = dsrPerBlock / assumedOneMinusReserveFactor,
    multiplierPerBlock;
...
describe("DAIInterestRateModelV2", () => {
  describe("constructor", () => {
    it.skip("sets jug and ilk address and pokes", async () => {
      ...
      let { contract: model } = await saddle.deployFull(
        ...
        { gas: 20000000, gasPrice: 20000, from: root },
        kovan,
      );
  ...
  describe("getBorrowRate", () => {
    [
      ...
      .forEach(([dsr, duty, base, cash, borrows, reserves = 0, jump = 0.8e18, kink = 0.9e18]) => {
        it(`calculates correct borrow value for dsr=${dsr / 1e25}%, duty=${duty / 1e25}%, base=${base / 1e25}%, jump=${
          jump / 1e18
        }, cash=${cash}, borrows=${borrows}, reserves=${reserves}`, async () => {
          ...
          const jug = await deploy("MockJug", [bnbUnsigned(onePlusPerSecondDuty), bnbUnsigned(perSecondBase)]);
          ...
          const expected = baseRoofRateFn(
            onePlusPerSecondDsr / 1e27,
            onePlusPerSecondDuty / 1e27,
            perSecondBase / 1e27,
            jump / 1e18,
            kink / 1e18,
            cash,
  ...
  describe("getSupplyRate", () => {
    [
      ...
      .map(vs => vs.map(Number))
      .forEach(
        ([dsr, duty, base, cash, borrows, reserves = 0, jump = 0.8e18, kink = 0.9e18, reserveFactor = 0.1e18]) => {
          it(`calculates correct supply value for dsr=${dsr / 1e25}%, duty=${duty / 1e25}%, base=${
            base / 1e25
          }%, cash=${cash}, borrows=${borrows}, reserves=${reserves}`, async () => {
            const onePlusPerSecondDsr = 1e27 + dsr / secondsPerYear;
            const onePlusPerSecondDuty = 1e27 + duty / secondsPerYear;
