...
async function governorBravoFixture(): Promise<GovernorBravoDelegateFixture> {
  ...
  const nwlVault = await smock.fake<XVSVault>("NWLVault");
  return { governorBravoDelegate, nwlVault };
}
...
describe("Governor Bravo Initializing Test", () => {
  beforeEach(async () => {
    ...
    ({ governorBravoDelegate, nwlVault } = contracts);
  ...
  describe("initilizer", () => {
    ...
    it("should revert if invalid nwl address", async () => {
      await expect(
        governorBravoDelegate.initialize(ethers.constants.AddressZero, [], [], ethers.constants.AddressZero),
      ).to.be.revertedWith("GovernorBravo::initialize: invalid nwl address");
    });
    it("should revert if invalid guardian address", async () => {
      await expect(
        governorBravoDelegate.initialize(nwlVault.address, [], [], ethers.constants.AddressZero),
    ...
    it("should revert if timelock adress count differs from governance routes count", async () => {
      ...
      await expect(
        governorBravoDelegate.initialize(nwlVault.address, [], timelocks, guardianAddress),
    ...
    it("should revert if proposal config count differs from governance routes count", async () => {
      ...
      await expect(
        governorBravoDelegate.initialize(nwlVault.address, proposalConfigs, timelocks, guardianAddress),
      ).to.be.revertedWith(
    ...
    it("should revert if initialized twice", async () => {
      ...
      const timelocks = [accounts[0].getAddress(), accounts[1].getAddress(), accounts[2].getAddress()];
      await governorBravoDelegate.initialize(nwlVault.address, proposalConfigs, timelocks, guardianAddress);
      await expect(
        governorBravoDelegate.initialize(nwlVault.address, proposalConfigs, timelocks, guardianAddress),
