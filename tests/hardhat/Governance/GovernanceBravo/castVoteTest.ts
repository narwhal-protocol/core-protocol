...
async function governorBravoFixture(): Promise<GovernorBravoDelegateFixture> {
  ...
  return { governorBravoDelegate, nwlVault, nwlStore, nwlToken };
}
...
describe("Governor Bravo Cast Vote Test", () => {
  beforeEach(async () => {
    ...
    ({ governorBravoDelegate, nwlVault, nwlToken } = contracts);
    ...
    await governorBravoDelegate.setVariable("nwlVault", nwlVault.address);
    await governorBravoDelegate.setVariable("proposalMaxOperations", 10);
  });
  describe("We must revert if:", () => {
    let customerAddress;
    it("We cannot propose without enough voting power by depositing nwl to the vault", async () => {
    ...
    describe("after we deposit nwl to the vault", () => {
      let proposalId: BigNumber;
      beforeEach(async () => {
        customerAddress = await customer.getAddress();
        const rootAddress = await root.getAddress();
        nwlToken.balanceOf.whenCalledWith(rootAddress).returns(400001);
        nwlVault.getPriorVotes.returns(convertToUnit("300000", 18));
