...
async function governorBravoFixture(): Promise<GovernorBravoDelegateFixture> {
  ...
  return { governorBravoDelegate, nwlVault, nwlStore, nwlToken, timelock };
}
...
describe("Governor Bravo State Tests", () => {
  ...
  beforeEach(async () => {
    ...
    ({ governorBravoDelegate, nwlVault, nwlToken, timelock } = contracts);
    ...
    nwlToken.balanceOf.returns(400001);
    nwlVault.getPriorVotes.returns(convertToUnit("600000", 18));
  ...
  it("Canceled", async () => {
    ...
    nwlVault.getPriorVotes.returns(convertToUnit("200000", 18));
