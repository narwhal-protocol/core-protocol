...
function configureNToken(vToken: FakeContract<NToken>, comptroller: MockContract<Comptroller>) {
  vToken.comptroller.returns(comptroller.address);
  vToken.isNToken.returns(true);
...
describe("Comptroller", () => {
  ...
  describe("Non zero address check", () => {
    ...
    testZeroAddress("_setNarwhalSpeed", [constants.AddressZero, 0]);
  });
  ...
  describe("_setCollateralFactor", () => {
    ...
    let comptroller: MockContract<Comptroller>;
    let vToken: FakeContract<NToken>;
    let oracle: FakeContract<PriceOracle>;

    type Contracts = SimpleComptrollerFixture & { vToken: FakeContract<NToken> };

    async function deploy(): Promise<Contracts> {
      const contracts = await deploySimpleComptroller();
      const vToken = await smock.fake<NToken>("NToken");
  ...
  describe("_supportMarket", () => {
    let comptroller: MockContract<Comptroller>;
    let oracle: FakeContract<PriceOracle>;
    let vToken1: FakeContract<NToken>;
    let vToken2: FakeContract<NToken>;
    let token: FakeContract<EIP20Interface>;

    type Contracts = SimpleComptrollerFixture & {
      vToken1: FakeContract<NToken>;
      vToken2: FakeContract<NToken>;
      token: FakeContract<EIP20Interface>;
    };

    async function deploy(): Promise<Contracts> {
      const contracts = await deploySimpleComptroller();
      const vToken1 = await smock.fake<NToken>("NToken");
      const vToken2 = await smock.fake<NToken>("NToken");
    ...
    beforeEach(async () => {
      ...
      configureOracle(oracle);
      configureNToken(vToken1, comptroller);
      configureNToken(vToken2, comptroller);
    });
  ...
  describe("Hooks", () => {
    ...
    beforeEach(async () => {
      ...
      configureNToken(vToken, comptroller);
    });
    ...
    describe("mintAllowed", () => {
      ...
      it("allows minting if cap is not reached", async () => {
        ...
        const currentNTokenSupply = convertToUnit("500", 18);
        const exchangeRate = convertToUnit("2", 18);
        // underlying supply = currentNTokenSupply * exchangeRate = 1000

        vToken.totalSupply.returns(currentNTokenSupply);
      ...
      it("reverts if market is not listed", async () => {
        const someNToken = await smock.fake<NToken>("NToken");
        await expect(comptroller.mintAllowed(someNToken.address, await root.getAddress(), convertToUnit("1", 18)))
