...
describe("VRTConverterProxy", () => {
  let root, alice, bob;
  let vrtConversion, vrtConversionAddress, vrtToken, vrtTokenAddress, nwlToken, nwlTokenAddress;
  let blockTimestamp;
  ...
  beforeEach(async () => {
    ...
    await send(vrtConversion, "setXVSVesting", [nwlVestingAddress]);
  ...
  describe("constructor", () => {
    ...
    it("sets decimalsMultiplier for  XVS", async () => {
      let nwlDecimalsMultiplierQueryResponse = await call(vrtConversion, "nwlDecimalsMultiplier");
      expect(parseInt(nwlDecimalsMultiplierQueryResponse)).toEqual(10 ** 18);
    });
    ...
    it("Fail on initialisation by non-Admin", async () => {
      await expect(
        send(
          vrtConversion,
          "initialize",
          [vrtTokenAddress, nwlTokenAddress, conversionRatio, conversionStartTime, conversionPeriod],
          { from: accounts[1] },
        ),
  ...
  describe("contract balances", () => {
    it("vrtConversion should be funded with XVS", async () => {
      const nwlTokenMintAmount = bnbMantissa(100000);
      await send(nwlToken, "transfer", [vrtConversionAddress, nwlTokenMintAmount], { from: root });

      let nwlTokenBalanceOfVRTConversion = await call(nwlToken, "balanceOf", [vrtConversionAddress]);
      expect(bnbUnsigned(nwlTokenBalanceOfVRTConversion)).toEqual(nwlTokenMintAmount);
    });
  ...
  describe("convert VRT to XVS", () => {
    ...
    it("alice can convert her VRT", async () => {
      ...
      let nwlVestedAmount = new BigNum(vrtTransferAmount).multipliedBy(new BigNum(conversionRatioMultiplier));
      expect(new BigNum(redeemableXVSAmountFromHarness)).toEqual(nwlVestedAmount);
      ...
      expect(convertVRTTxn).toHaveLog("TokenConverted", {
        ...
        nwlAddress: nwlTokenAddress,
        nwlAmount: nwlVestedAmount.toFixed(),
      });
    ...
    it("alice can make multiple VRT conversions", async () => {
      ...
      let nwlVestedAmount_Conversion_1 = new BigNum(vrtTransferAmount_Conversion_1).multipliedBy(
        new BigNum(conversionRatioMultiplier),
      );
      expect(new BigNum(redeemableXVSAmountFromHarness_Conversion_1)).toEqual(nwlVestedAmount_Conversion_1);
      ...
      expect(convertVRTTxn).toHaveLog("TokenConverted", {
        ...
        nwlAddress: nwlTokenAddress,
        nwlAmount: nwlVestedAmount_Conversion_1.toFixed(),
      });
      ...
      let nwlVestedAmount_Conversion_2 = new BigNum(vrtTransferAmount_Conversion_2).multipliedBy(
        new BigNum(conversionRatioMultiplier),
      );
      expect(new BigNum(redeemableXVSAmountFromHarness_Conversion_2)).toEqual(nwlVestedAmount_Conversion_2);
      ...
      expect(convertVRTTxn_2).toHaveLog("TokenConverted", {
        ...
        nwlAddress: nwlTokenAddress,
        nwlAmount: nwlVestedAmount_Conversion_2.toFixed(),
      });
