const { makeChainlinkOracle, makeNToken } = require("./Utils/Narwhal");

const { increaseTime, bnbMantissa } = require("./Utils/BSC");

describe("NarwhalChainlinkOracle", () => {
  let root, accounts;
  let bnbFeed, daiFeed, usdcFeed, usdtFeed;
  let oracle, vBnb, vDai, vExampleSet, vExampleUnset, vUsdc, vUsdt, vai, nwl;
  ...
  beforeEach(async () => {
    ...
    await makeNToken();
    vBnb = await makeNToken({ kind: "vbnb", comptrollerOpts: { kind: "v1-no-proxy" }, supportMarket: true });
    vai = await makeNToken({
      comptroller: vBnb.comptroller,
      supportMarket: true,
      symbol: "VAI",
    });
    nwl = await makeNToken({
      comptroller: vBnb.comptroller,
      supportMarket: true,
      symbol: "XVS",
    });
    vExampleSet = await makeNToken({
      comptroller: vBnb.comptroller,
      supportMarket: true,
    });
    vExampleUnset = await makeNToken({
      comptroller: vBnb.comptroller,
      supportMarket: true,
    });
    vUsdc = await makeNToken({
      comptroller: vBnb.comptroller,
      supportMarket: true,
      underlyingOpts: {
        decimals: 6,
    ...
    oracle = await deploy("NarwhalChainlinkOracle", [MAX_STALE_PERIOD]);
  });
  ...
  describe("getUnderlyingPrice", () => {
    beforeEach(async () => {
      ...
      await send(oracle, "setDirectPrice", [nwl._address, 7], { from: root });
  ...
  describe("setDirectPrice", () => {
    it("only admin may set an underlying price", async () => {
      await expect(send(oracle, "setDirectPrice", [nwl._address, 7], { from: accounts[0] })).rejects.toRevert(
        "revert only admin may call",
      );
    });
    ...
    it("sets the direct price", async () => {
      await send(oracle, "setDirectPrice", [nwl._address, 7], { from: root });
      let price = await call(oracle, "assetPrices", [nwl._address], { from: root });
      expect(price).toEqual("7");
    });
  });
