...
async function makeComptroller(opts = {}) {
  ...
  if (kind == "bool") {
    ...
    const nwl = opts.nwl || (await deploy("XVS", [opts.narwhalOwner || root]));
    ...
    return Object.assign(comptroller, { nwl, vai, vaicontroller: vaiunitroller });
  }
  ...
  if (kind == "unitroller-g2") {
    ...
    const nwl = opts.nwl || (await deploy("XVS", [opts.compOwner || root]));
    const narwhalRate = bnbUnsigned(dfn(opts.narwhalRate, 1e18));
    ...
    await send(unitroller, "harnessSetNarwhalRate", [narwhalRate]);
    await send(unitroller, "setXVSAddress", [nwl._address]); // harness only

    return Object.assign(unitroller, { priceOracle, nwl });
  }
  ...
  if (kind == "unitroller") {
    ...
    return Object.assign(unitroller, { priceOracle, nwl, vai, vaiunitroller });
  }
}
...
async function makeNToken(opts = {}) {
  ...
  switch (kind) {
    ...
    case "vnwl":
      ...
      vDelegatee = await deploy("VNwlLikeDelegate");
      vDelegator = await deploy("VBep20Delegator", [
        underlying._address,
        comptroller._address,
        interestRateModel._address,
      ...
      vToken = await saddle.getContractAt("VNwlLikeDelegate", vDelegator._address);
      break;
  ...
  if (opts.addNarwhalMarket) {
    await send(comptroller, "_addNarwhalMarket", [vToken._address]);
  }
