const { makeComptroller, makeNToken, enterMarkets, quickMint, setMarketSupplyCap } = require("../Utils/Narwhal");
...
describe("Comptroller", () => {
  ...
  describe("liquidity", () => {
    it("fails if a price has not been set", async () => {
      const vToken = await makeNToken({ supportMarket: true });
    ...
    it("allows a borrow up to collateralFactor, but not more", async () => {
      const collateralFactor = 0.5,
        ...
        user = accounts[1],
        amount = 1e6;
      const vToken = await makeNToken({ supportMarket: true, collateralFactor, underlyingPrice });
    ...
    it("allows entering 3 markets, supplying to 2 and borrowing up to collateralFactor in the 3rd", async () => {
      ...
      const vToken1 = await makeNToken({ supportMarket: true, collateralFactor: cf1, underlyingPrice: up1 });
      await setMarketSupplyCap(vToken1.comptroller, [vToken1._address], [100000000000]);
      const vToken2 = await makeNToken({
        supportMarket: true,
      ...
      const vToken3 = await makeNToken({
        supportMarket: true,
        comptroller: vToken1.comptroller,
  ...
  describe("getHypotheticalAccountLiquidity", () => {
    ...
    it("returns collateral factor times dollar amount of tokens minted in a single market", async () => {
      const collateralFactor = 0.5,
        exchangeRate = 1,
        underlyingPrice = 1;
      const vToken = await makeNToken({ supportMarket: true, collateralFactor, exchangeRate, underlyingPrice });
      await setMarketSupplyCap(vToken.comptroller, [vToken._address], [100000000000]);
