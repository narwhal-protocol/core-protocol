...
const { makeNToken, balanceOf, borrowSnapshot, enterMarkets, setMarketSupplyCap } = require("./Utils/Narwhal");
...
describe("Spinarama", () => {
  ...
  describe("#mintMint", () => {
    it("should succeed", async () => {
      const vToken = await makeNToken({ supportMarket: true, underlyingPrice: 1 });
  ...
  describe("#repayRepay", () => {
    it("should succeed", async () => {
      const vToken1 = await makeNToken({ supportMarket: true, underlyingPrice: 1, collateralFactor: 0.5 });
      await setMarketSupplyCap(vToken1.comptroller, [vToken1._address], [100000000000]);
      const vToken2 = await makeNToken({ supportMarket: true, underlyingPrice: 1, comptroller: vToken1.comptroller });
